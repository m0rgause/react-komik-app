...
const Search = () => {
    ...
    useEffect(() => {
        axios.get(`${config.server}/search/?query=${query}&page=${page}`).then((res) => {
                        ...
                        <div className="d-flex justify-content-center my-4">
                            <ul className="pagination">
                                <li className={page <= 1 ? 'page-item disabled' : 'page-item'}><a className="page-link title" style={{
                                    color: "var(--bs-emphasis-color)",
                                    backgroundColor: "var(--bs-secondary-bg)",
                                }} href={`/search/${parseInt(page) - 1}?s=${query}`}>← Sebelumnya</a></li>
                                <li className={page >= maxPage ? 'page-item disabled' : 'page-item'}><a className={"page-link title"} style={{
                                    color: "var(--bs-emphasis-color)",
                                    backgroundColor: "var(--bs-secondary-bg)",
                                }} href={`/search/${parseInt(page) + 1}?s=${query}`}>Selanjutnya →</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
